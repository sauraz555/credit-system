...
export default function AnalystWorkspace() {
      ...
      setDisputes([
        {
          ...
          subject_name: 'Jonathan Edward Vance',
          target_listing: 'Telstra Default ($420.00)',
          grounds: 'Section 6Q notice not delivered prior to bureau listing.',
        ...
        {
          id: 'DISP-2026-0038',
          entity_id: 'IND-9021-1981',
          subject_name: 'Sarah Jane Connor',
          target_listing: 'ANZ Overdue Account ($1,250.00)',
          grounds: 'Part IIIA Hardship arrangement active; listing unlawful.',
  ...
  const fetchModels = async () => {
    try {
      ...
      setModels([
        { id: 'v1', name: 'CCR Baseline Model v1.0', type: 'BASELINE' },
        { id: 'v2', name: 'Challenger Model v2.1-Alpha', type: 'CHALLENGER' },
      ]);
  ...
  const triggerBacktest = async () => {
    ...
    try {
      ...
      formData.append('model_id', selectedModelId || 'baseline');
      ...
      setTimeout(() => {
        setBacktestResults({
          ...
          total_records: 1250,
          band_performance: [
            { band: 'Below 500 (Subprime)', count: 180, defaults: 92, default_rate: 0.511 },
            { band: '500-619 (Near Prime)', count: 320, defaults: 64, default_rate: 0.20 },
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#999999] mt-1">
            Statutory dispute adjudication, bitemporal historical file investigation, and scoring model back-testing.
          </p>
        </div>
      ...
      <div className="bg-[#141417] border border-[#202026] rounded-[2px]">
        <Tabs selectedIndex={selectedTab} onChange={({ selectedIndex }) => setSelectedTab(selectedIndex)}>
          <TabList aria-label="Analyst Workspace Navigation" className="bg-[#1c1c21] border-b border-[#202026]">
            <Tab renderIcon={DocumentView} className="text-xs font-semibold">1. Dispute Queue (Sec 20V)</Tab>
            <Tab renderIcon={Time} className="text-xs font-semibold">2. Historical File Investigation</Tab>
            <Tab renderIcon={Analytics} className="text-xs font-semibold">3. Model Back-Testing</Tab>
          </TabList>
          ...
          <TabPanels>
            ...
            <TabPanel className="p-5 md:p-6 space-y-4">
              ...
              {loadingDisputes ? (
                ...
                <div className="py-8 text-center text-[#777777] border border-dashed border-[#202026] rounded-[2px]">
                  ...
                  <p className="font-medium text-xs text-[#e6e6e6]">No Pending Disputes</p>
                  <p className="text-[11px] mt-1">All statutory investigations have been resolved or closed.</p>
                </div>
              ) : (
            ...
            <TabPanel className="p-5 md:p-6 space-y-4">
              ...
              {loadingInvestigation ? (
                  ...
                  <div className="border border-[#202026] p-4 bg-[#1c1c21] rounded-[2px] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      ...
                      <div className="text-xs text-[#999999] font-mono mt-1">
                        Subject ID: {investigationData.entity_id} &bull; Model: {investigationData.score?.model_name || 'CCR Baseline v1'}
                      </div>
                    </div>

                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <div className="text-[10px] text-[#777777] uppercase font-semibold">Point-in-Time Score</div>
                        ...
                        <table className="w-full text-left text-xs">
                          <thead>
                            <tr className="bg-[#1c1c21] border-b border-[#202026] text-[#999999]">
                              ...
                              <th className="p-2.5 font-mono">Amount</th>
                              <th className="p-2.5 font-mono">Valid From</th>
                              <th className="p-2.5 font-mono">Recorded At</th>
                ...
                <div className="py-8 text-center text-[#777777] border border-dashed border-[#202026] rounded-[2px]">
                  ...
                  <p className="font-medium text-xs text-[#e6e6e6]">Enter Subject ID and Observation Date</p>
            ...
            <TabPanel className="p-5 md:p-6 space-y-4">
              <div className="pb-3 border-b border-[#202026]">
                ...
                <p className="text-xs text-[#999999] mt-0.5">
                  Evaluate predictive accuracy against empirical default outcomes. Computes ROC AUC, Gini index, and Kolmogorov-Smirnov separation.
                </p>
              </div>
              ...
              <div className="p-4 bg-[#1c1c21] border border-[#202026] rounded-[2px] grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                ...
                <div>
                  <label className="block text-xs font-medium text-[#999999] mb-1">
                    Outcomes CSV (entity_id, outcome, defaulted)
                  </label>
                  <input
                    type="file"
                    accept=".csv"
                ...
                <div>
                  <Button
                    ...
                    className="w-full"
                  >
                    {runningBacktest ? 'Computing...' : 'Run Test'}
                  </Button>
                </div>
              </div>
              ...
              {runningBacktest ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="p-3 border border-[#202026] bg-[#1c1c21] rounded-[2px]">
                      <div className="text-[10px] text-[#777777] uppercase font-semibold">ROC AUC</div>
                      <div className="text-xl font-mono font-bold text-[#0f62fe] mt-1">
                        {backtestResults.auc}
                      </div>
                      <div className="text-[10px] text-[#999999] mt-0.5">
                        {backtestResults.auc >= 0.7 ? 'Strong Discrimination' : 'Moderate'}
                      </div>
                    </div>

                    <div className="p-3 border border-[#202026] bg-[#1c1c21] rounded-[2px]">
                      <div className="text-[10px] text-[#777777] uppercase font-semibold">Gini Coefficient</div>
                    ...
                    <div className="p-3 border border-[#202026] bg-[#1c1c21] rounded-[2px]">
                      <div className="text-[10px] text-[#777777] uppercase font-semibold">KS Statistic</div>
                      <div className="text-xl font-mono font-bold text-[#f1c21b] mt-1">
                        {backtestResults.ks_statistic}
                      </div>
                      <div className="text-[10px] text-[#999999] mt-0.5">Max CDF Separation</div>
                    </div>

                    <div className="p-3 border border-[#202026] bg-[#1c1c21] rounded-[2px]">
                      <div className="text-[10px] text-[#777777] uppercase font-semibold">Records Evaluated</div>
                      <div className="text-xl font-mono font-bold text-[#e6e6e6] mt-1">
                        {backtestResults.total_records}
                      </div>
                      <div className="text-[10px] text-[#999999] mt-0.5">Ground Truth Pairs</div>
                    </div>
                  </div>
                  ...
                  <div className="border border-[#202026] rounded-[2px] overflow-x-auto">
                    <div className="p-3 bg-[#1c1c21] border-b border-[#202026] font-semibold text-xs text-[#e6e6e6]">
                      Decile Calibration Table (Rank Ordered Lowest to Highest Score)
                    </div>
                    <table className="w-full text-left text-xs">
                      <thead>
                        <tr className="border-b border-[#202026] bg-[#141417] text-[#999999]">
                          <th className="p-2.5">Decile</th>
                          <th className="p-2.5 font-mono">Score Range</th>
                          <th className="p-2.5 font-mono">Entities</th>
                          <th className="p-2.5 font-mono">Defaults</th>
                          <th className="p-2.5 font-mono">Observed Default Rate</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#202026]">
                        {backtestResults.deciles?.map((d: any) => (
                          <tr key={d.decile} className="hover:bg-[#1c1c21] transition-colors">
                            ...
                            <td className="p-2.5 font-mono text-[#999999]">
                              {d.score_min} &ndash; {d.score_max}
                            </td>
                            <td className="p-2.5 font-mono text-[#999999]">{d.count}</td>
                ...
                <div className="py-8 text-center text-[#777777] border border-dashed border-[#202026] rounded-[2px]">
                  ...
                  <p className="font-medium text-xs text-[#e6e6e6]">No Backtest Results Generated</p>
                  <p className="text-[11px] mt-1">
